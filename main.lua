...
---@return File|nil, boolean|nil
local current_file = ya.sync(function()
	local h = cx.active.current.hovered
	if not h then
		return
	end
	return h, h.cha.is_dir
end)
...
local function run_command(cmd, args, _stdin)
	...
	if not cwd.spec then
		...
		if cwd.spec.is_virtual then
			if hovered_file and hovered_file.path and hovered_file.path.parent then
				cwd = tostring(hovered_file.path.parent)
			else
...
return {
	entry = function(_, job)
		...
		if action == "mount" then
			...
			local hovered_file = hovered_url and fs.file(hovered_url)
			if not hovered_url then
				hovered_file, is_dir = current_file()
				hovered_url = hovered_file.url
			end
			if hovered_file == nil or hovered_url == nil then
				return
			end
			...
			if not hovered_url.spec then
				...
				hovered_url_cached = is_virtual and Url(hovered_file.path) or hovered_url.path or hovered_url
			end
